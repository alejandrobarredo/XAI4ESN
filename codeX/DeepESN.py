...
class DeepESN():
    ...
    def __init__(self, Nu, Nr, Nl, configs, verbose=0):
        ...
        rhos = np.array(
            configs['rhos'])  # spectral radius (maximum absolute eigenvalue)
            ...
            for layer in range(Nl):
                ...
                for row in range(Nr):
                    number_row_elements = round(
                        reservoirConf['connectivity'] * Nr)
                    row_elements = random.sample(range(Nr),
                                                 number_row_elements)
                    self.W[layer][row, row_elements] = np.random.uniform(-1,
                                                                         +1,
                                                                         size=(
                                                                             1,
                                                                             number_row_elements))
        ...
        for layer in range(Nl):
            ...
            if layer == 0:
                self.Win[layer] = np.random.uniform(-input_scale, input_scale,
                                                    size=(Nr, Nu + 1))
            else:
                self.Win[layer] = np.random.uniform(-input_scale, input_scale,
                                                    size=(Nr, Nr + 1))
    ...
    def computeDeepIntrinsicPlasticity(self, inputs):
        ...
        for layer in range(self.Nl):
            ...
            for epoch in range(self.IPconf.Nepochs):
                ...
                if (np.linalg.norm(self.Gain[layer] - Gain_epoch,
                                   2) < self.IPconf.threshold) and (
                        np.linalg.norm(self.Bias[layer] - Bias_epoch,
                                       2) < self.IPconf.threshold):
            ...
            for i in range(len(inputs)):
                states[i][(layer) * self.Nr: (layer + 1) * self.Nr, :] = \
                    inputs2[i]
    ...
    def computeState(self, inputs, DeepIP=0, initialStates=None, verbose=0):
        ...
        if self.IPconf['DeepIP'] and DeepIP:
            ...
            for i_seq in range(len(inputs)):
                states.append(
                    self.computeGlobalState(inputs[i_seq], initialStates))
    ...
    def computeGlobalState(self, input, initialStates):
        ...
        for layer in range(self.Nl):
            if initialStates is not None:
                initialStatesLayer = initialStates[
                                     (layer) * self.Nr: (layer + 1) * self.Nr,
                                     :]
            state[(layer) * self.Nr: (layer + 1) * self.Nr,
            :] = self.computeLayerState(input, layer, initialStatesLayer, 0)
            input = state[(layer) * self.Nr: (layer + 1) * self.Nr, :]

        return state

    def computeLayerState(self, input, layer, initialStatesLayer=None,
                          DeepIP=0):
        ...
        input = self.Win[layer][:, 0:-1].dot(input) + np.expand_dims(
            self.Win[layer][:, -1], 1)

        if DeepIP:
            state_net = np.zeros((self.Nr, input.shape[1]))
            state_net[:, 0:1] = input[:, 0:1]
            state[:, 0:1] = self.lis[layer] * np.tanh(
                np.multiply(self.Gain[layer], state_net[:, 0:1]) + self.Bias[
                    layer])
        else:
            # state[:,0:1] = self.lis[layer] * np.tanh(np.multiply(self.Gain[layer], input[:,0:1]) + self.Bias[layer])
            state[:, 0:1] = (1 - self.lis[layer]) * initialStatesLayer + \
                            self.lis[layer] * np.tanh(
                np.multiply(self.Gain[layer],
                            self.W[layer].dot(initialStatesLayer) + input[:,
                                                                    0:1]) +
                self.Bias[layer])

        for t in range(1, state.shape[1]):
            if DeepIP:
                state_net[:, t:t + 1] = self.W[layer].dot(
                    state[:, t - 1:t]) + input[:, t:t + 1]
                state[:, t:t + 1] = (1 - self.lis[layer]) * state[:, t - 1:t] + \
                                    self.lis[layer] * np.tanh(
                    np.multiply(self.Gain[layer], state_net[:, t:t + 1]) +
                    self.Bias[layer])
                ...
                # IP learning rule
                deltaBias = -eta * (
                        (-mu / sigma2) + np.multiply(state[:, t:t + 1], (
                        2 * sigma2 + 1 - (
                        state[:, t:t + 1] ** 2) + mu * state[:,
                                                       t:t + 1]) / sigma2))
                deltaGain = eta / npm.repmat(self.Gain[layer], 1,
                                             state_net[:, t:t + 1].shape[
                                                 1]) + deltaBias * state_net[:,
                                                                   t:t + 1]
                ...
                state[:, t:t + 1] = (1 - self.lis[layer]) * state[:, t - 1:t] + \
                                    self.lis[layer] * np.tanh(
                    np.multiply(self.Gain[layer],
                                self.W[layer].dot(state[:, t - 1:t]) + input[:,
                                                                       t:t + 1]) +
                    self.Bias[layer])

        return state

    def trainReadout(self, trainStates, trainTargets, lb=10.0 ** -2, alpha=5, verbose=0, multiclass=False,
                     class_balance=None, val_data=None):
        if self.readout['trainMethod'] == 'MLP' or self.readout['trainMethod'] == 'Forest':
            trainTargets = np.array(trainTargets)
            trainStates = np.stack(trainStates, axis=0).reshape(len(trainStates), -1)

        if self.readout['trainMethod'] == 'MLP':
            self.scaler.fit(trainStates)
            trainStates = self.scaler.transform(trainStates)
            Y_train = to_categorical(trainTargets, np.max(trainTargets) + 1)
            model = Sequential()
            model.add(Dense(1000, input_shape=(trainStates.shape[1],), activation='relu'))
            #model.add(Dropout(0.5))
            model.add(Dense(100, activation='relu'))
            model.add(Dense(np.max(trainTargets) + 1, activation='softmax'))
            model.compile(loss='categorical_crossentropy', optimizer='SGD', metrics=['accuracy'])
            model.fit(trainStates, np.squeeze(Y_train),
                      epochs=40,
                      batch_size=128,
                      verbose=1,
        ...
        if self.readout['trainMethod'] == 'Forest':
            clf = ExtraTreesClassifier(n_estimators=100,
                                       ...
                                       n_jobs=10,
                                       random_state=0)
            clf.fit(trainStates, trainTargets)
            self.predictor.append(clf)

        if self.readout['trainMethod'] == 'Lasso':
            clf = LogisticRegression(solver='liblinear', C=0.1, penalty='l1', tol=1e-1, class_weight=class_balance)
            clf.fit(trainStates.T, np.squeeze(trainTargets.astype(np.int32)))
            self.predictor.append(clf)

        if self.readout['trainMethod'] == 'Inverse':
            trainStates = np.concatenate(trainStates, 1)
            trainTargets = trainTargets.reshape(1, -1)
            ...
            self.Wout = np.linalg.solve((A + np.eye(A.shape[0],
                                                    A.shape[1]) * lb), B.T).T

        # else:
        #     trainTargets = np.tile(trainTargets, (50, 1)).reshape(-1,)
        #     trainStates = np.concatenate(trainStates, 1)
    ...
    def computeOutput(self, state, probs=False):

        if self.readout['trainMethod'] == 'MLP':
            state = np.stack(state, axis=0).reshape(len(state), -1)
            state = self.scaler.transform(state)
            return np.argmax(self.predictor[0].predict(state), axis=1)

        if self.readout['trainMethod'] == 'Forest':
            state = np.stack(state, axis=0).reshape(len(state), -1)
            if not probs:
                return self.predictor[0].predict(state)
            else:
                return self.predictor[0].predict_proba(state)

        if self.readout['trainMethod'] == 'Inverse':
            return self.Wout[:, 0:-1].dot(state) \
                   + np.expand_dims(self.Wout[:, -1], 1)

        if self.readout['trainMethod'] == 'Lasso':
            state = np.concatenate(state, 1)
            if not probs:
                preds = np.mean(self.predictor[0].predict(state.T).reshape(50, -1), axis=0)
                preds[preds < 0.5] = 0
                preds[preds >= 0.5] = 1
                return preds
            else:
                return np.mean(self.predictor[0].predict(state.T).reshape(50, -1), axis=0)
